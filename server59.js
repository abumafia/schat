...
// LiveKit (SFU) for reliable realtime voice/video (TURN handled by LiveKit Cloud)
let AccessToken;
try {
  ({ AccessToken } = require('livekit-server-sdk'));
} catch (e) {
  console.warn('⚠️ livekit-server-sdk not installed. Run: npm i livekit-server-sdk');
}
...
// ==================== LIVEKIT TOKEN (REALTIME CALLS) ====================
// Client requests a token to join a LiveKit room. Auth is required.
// .env must include:
//   LIVEKIT_WS_URL=wss://xxxx.livekit.cloud
//   LIVEKIT_API_KEY=xxxx
//   LIVEKIT_API_SECRET=xxxx
app.get('/api/livekit/token', authenticateToken, async (req, res) => {
  try {
    if (!AccessToken) {
      return res.status(500).json({ error: 'LiveKit server SDK missing. Install livekit-server-sdk.' });
    }
    ...
    if (!apiKey || !apiSecret || !url) {
      return res.status(500).json({ error: 'LiveKit env vars missing (LIVEKIT_WS_URL, LIVEKIT_API_KEY, LIVEKIT_API_SECRET)' });
    }
