...
const PET_MARKET = {
  ...
  outfits: [
    ...
    { id: 'outfit_sky', name: 'Sky kiyim', color: '#0ea5e9', price:
...
app.post('/api/robots/play', authenticateToken, async (req, res) => {
  try {
    ...
    const active = target.robots.find(r => String(r._id) === String(target.activeRobotId)) || target.robots[0];

    // Reaksiya: yoqimtoylikka qarab
    const c = Math.max(0, Math.min(100, active.cuteness || 50));
    let reaction = 'neutral';
    let text = 'Robotcha sizga qaradi.';
    if (action === 'wave') { reaction = (c > 60 ? 'happy' : 'neutral'); text = (c > 60 ? 'Robotcha xursand bo‘lib qo‘l silkitdi.' : 'Robotcha ehtiyotkorlik bilan qo‘l silkitdi.'); }
    if (action === 'tickle') { reaction = (c > 75 ? 'happy' : 'thinking'); text = (c > 75 ? 'Robotcha kulib yubordi.' : 'Robotcha biroz o‘ylab qoldi.'); }
    if (action === 'boop') { reaction = (c > 55 ? 'happy' : 'sad'); text = (c > 55 ? 'Robotcha “boop”dan xursand bo‘ldi.' : 'Robotcha biroz xafa bo‘lib qoldi.'); }
    if (action === 'highfive') { reaction = (c > 65 ? 'happy' : 'neutral'); text = (c > 65 ? 'Robotcha high-five berdi!' : 'Robotcha sekin high-five qildi.'); }

    active.mood = reaction;
    await target.save();

    res.json({ success: true, reaction, message: text, targetRobot: { id: String(active._id), typeId: active.typeId, name: active.name, cuteness: active.cuteness, level: active.level, mood: active.mood } });
  } catch (e) {
