...
const io = socketIO(server, {
  ...
  // Prevent short disconnects that can drop calls on flaky mobile networks
  pingInterval: 25000,
  pingTimeout: 90000,
...
// NOTE: For full reliability on mobile networks / symmetric NAT you MUST provide a TURN server.
// Set env:
//   TURN_URL=turn:your.turn.server:3478
//   TURN_USERNAME=...
//   TURN_PASSWORD=...
// You may also use turns:... with 5349.
app.get('/api/webrtc-config', (req, res) => {
  ...
  if (turnUrl && turnUser && turnPass) {
    iceServers.unshift({ urls: [turnUrl], username: turnUser, credential: turnPass });
  }

  res.json({
    iceServers,
    // Client can tune aggressiveness
    iceCandidatePoolSize: 10
  });

// Global call notification widget script
app.get('/callkit.js', (req, res) => {
  res.setHeader('Content-Type', 'application/javascript; charset=utf-8');
  res.sendFile(path.join(__dirname, 'callkit.js'));
});
...
// ICE restart offer (for flaky networks; keeps calls alive)
socket.on('iceRestartOffer', (data) => {
  try {
    const payload = { ...data, from: socket.userId, timestamp: Date.now() };
    emitToUser(data.to, 'iceRestartOffer', payload);
  } catch (e) {
    console.error('iceRestartOffer error:', e);
  }
});

// ICE restart answer
socket.on('iceRestartAnswer', (data) => {
  try {
    const payload = { ...data, from: socket.userId, timestamp: Date.now() };
    emitToUser(data.to, 'iceRestartAnswer', payload);
  } catch (e) {
    console.error('iceRestartAnswer error:', e);
  }
});
